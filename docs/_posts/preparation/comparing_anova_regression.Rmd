...
df <- psycho::emotion %>% 
  select(Participant_ID, Recall, Emotion_Condition, Subjective_Arousal)
```


# Comparing RM with MM ANOVAs

repeated measures with mixed-models based ANOVAs.

```{r, fig.width=7, fig.height=4.5, eval = TRUE, results='markup', fig.align='center', comment=NA, message=FALSE, warning=FALSE}
results_anova <- aov(Subjective_Arousal ~ Recall * Emotion_Condition + Error(Participant_ID), data=df)
results_lmer <- lmerTest::lmer(Subjective_Arousal ~ Recall * Emotion_Condition + (1|Participant_ID), data=df)
results_lmer <- anova(results_lmer, type=1)
```
```{r, fig.width=7, fig.height=4.5, eval = TRUE, results='markup', fig.align='center', comment=NA, message=FALSE, warning=FALSE}
analyze(results_anova)
analyze(results_lmer)
```

# Comparing repeated measures ANOVA with Mixed Models

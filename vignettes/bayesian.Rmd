...
ggplot(data.frame(x = density$x, y = density$y), aes(x=x, y=y)) +
  ...
  annotate("segment", x = r+0.03, xend = r, y = 10, yend = 10, size=0.1, arrow=arrow(type="closed", length = unit(0.10, "inches")), color="#4CAF50") +
  annotate("text", x = r+0.035, y = 10, label = "Frequentist r Coefficient" , size=4 , fontface="bold", hjust = 0, color="#4CAF50") +
  # median
  geom_vline(xintercept=median(posterior)) +
  annotate("segment", x = median(posterior)+0.03, xend = r, y = 8, yend = 8, colour = "black", size=0.1, arrow=arrow(type="closed", length = unit(0.10, "inches"))) +
  annotate("text", x = median(posterior)+0.035, y = 8, label = "Median" , size=4 , fontface="bold", hjust = 0) +
  # # mean
...
# Generate a new refgrid with fewer observations
ref_grid <- emmeans::ref_grid(fit, at = list(
  Tolerating = seq(2)))  

# Get predictions and keep iterations
predicted <- psycho::get_predicted(fit, refgrid=ref_grid, posterior_predict=FALSE, keep_iterations=TRUE)
...
ggplot(predicted, aes(x=Tolerating)) +
  geom_line(aes(y=Iteration_Value, group=Iteration), size=0.3, alpha=0.02) +
  geom_line(aes(y=pred_Life_Satisfaction), size=2) + 
...
predicted$pred_Sex_5_proba <- psycho::odds_to_probs(predicted$pred_Sex_5)
predicted$pred_Sex_95_proba <- psycho::odds_to_probs(predicted$pred_Sex_95)
```
...
ggplot(predicted, aes(x=Adjusting, y=pred_Sex_proba)) +
  geom_line() +
  geom_ribbon(aes(ymin=pred_Sex_5_proba, 
                  ymax=pred_Sex_95_proba), 
              alpha=0.1) +

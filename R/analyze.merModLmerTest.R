...
analyze.merModLmerTest <- function(x, ...) {
  ...
  fitsum <- select_(fitsum, "Coef", "SE", "t", "df", "Coef.std", "SE.std",
                    "p", "Effect_Size")
  ...
  for (varname in varnames){
    text <- paste("The effect of ", varname, " was [NOT] significant
                  (beta = ", format_digit(fitsum[varname, "Coef"], 2),
                  ", SE = ", format_digit(fitsum[varname, "SE"], 2),
                  ", t(", format_digit(fitsum[varname, "df"], 2), ") = ",
                  ...
                  ", std. SE = ", format_digit(fitsum[varname, "SE.std"], 2),
                  ").", sep="")
  ...
  text <- c(paste("The overall model predicting ... successfully converged
                  and explained ", format_digit(R2c * 100, 2), "% of the
                  variance of the endogen (the conditional R2). ", "The
                  variance explained by the fixed effects was of ",
